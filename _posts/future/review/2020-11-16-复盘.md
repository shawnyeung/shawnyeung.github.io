...
date: 2020-11-16
categories: [期货,复盘]
tags: [RS,MM,PINBAR,GAP,TR,FBO，MTR]
---
...
　　开盘后在`5`出现假突破，但被`6`反转，`7`和`9`连续回测没过，说明**假突破成立**，反转成功。则后续**反转的measured move目标位为去掉假突破部分的open range的2倍**。最后在`23`差一个tick到达。
...
## 7 趋势中途出现停顿的平台，则趋势恢复后的MM目标位

　　MTR中途出现`41`到`46`停顿的平台，则趋势恢复后，**measured move目标位为趋势顶点（`32`的H）到平台（`46`的L）的2倍位置**。恰好也是前低，最后在`51`一分不差到达。
